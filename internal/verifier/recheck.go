...
func (verifier *Verifier) insertRecheckDocs(
	...
	for i, dbName := range dbNames {
		recheckRaw := bsoncore.NewDocumentBuilder().
			AppendDocument("_id", bsoncore.NewDocumentBuilder().
				AppendString("db", dbName).
				AppendString("coll", collNames[i]).
				AppendValue("docID", bsoncore.Value{
					Type: rawDocIDs[i].Type,
					Data: rawDocIDs[i].Value,
				}).
				Build()).
			Build()
		...
		if curBatchBytes > recheckBatchByteLimit || len(recheckRaw) >= recheckBatchCountLimit {
