...
// GetCursorExtra returns the most recent response’s metadata.
// This includes, e.g., the `operationTime`.
func (c Cursor) GetExtra() ExtraMap {
	return maps.Clone(c.curExtra)
}
...
func (c *Cursor) GetNext(ctx context.Context, extraPieces ...bson.E) error {
	...
	raw, err := resp.Raw()
	if err != nil {
		return errors.Wrapf(err, "%#q’s cursor iteration failed", c.ns)
	}
